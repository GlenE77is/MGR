...
#! 
export gcSysDevDob="240116-1917"    # recovered
#!
export gcMgrDevId="Mgr{S}" # "R" "S" "T" 
#! 
#! ######################################################################################
...
#! top: Fall-Thru "initz" function 
#!
#! Splash / Indicator for "RUNNING" 
#~ export gcMgrDevId="Manager:" # "I" "d" 
#!
lcMsgStr="${gcMgrDevId}"
...
#!
#! will kill all GtkPid_Sbr_Init_Splash at top of main ()
#!
#! end: Fall-Thru "initz" function 
#!
#!
...
#!
    ...
    #! =================================================
    #!~ iso-KineticPup32-22.10+2_______________230604
    #!~ iso-BionicPup32-8.0+30_BionicBeaver___K:4_220407
    #!~ iso_BionicPup32-19.03-K5.9_231209.iso_K:5_231209
    #!~ iso-UPupFF+D-20.12+4_Focal-Fossa_______201210
    #!~ iso_BookwormPup32-23.11-B4_K6.1.55 (i686)_220227_231210
    #!~ iso_ManticPup32-23.10+1.iso____________231209
    #!~ iso-JammyPup32-22.04+8_JammyJellyfish__221203
...
#!
export gcSysNameBase="MGR.6.0"  #: used in ${gcSysNameBase} code as systemlabel
...
#!
#! ### MAINGUI # MGS ### Window SetUp ###########################################
export gcMgrVer="-6.3-Dev"   #"-lint" "-TXN" "-DEV"
#! ###############################
#!
#! ### gcSysKillProc #####################
...
export lcGuiColoryel="yellow"
export lcGuiColordrkyel="darkyellow"
#!
#! test these:
...
#(: ============================================================
#(: fx(Sbr_CDW_Embedded_Dialog_Colors)	#: Active
#(: called by:
#(: purpose: 
#(: receives param:
#(: returns:
...
#(:
#! ###########################################################
function Sbr_CDW_Embedded_Dialog_Colors() { # marker
    n=0
...
#!
#(: ============================================================
#(: fx(Sbr_CDW_EGI_Short)	#: Active
#(: called by: main ()
#(: purpose: (MGR) CDW EGI SHORT FORM
#(: receives param:
#(: returns:
#(: note: called by CheckIF 
#(: note: copied from MCI 
#(:
#! ############################################################
function Sbr_CDW_EGI_Short() {  # SHORT FORM CheckIF # to be Not Called 
	#! long form has been copied to rc.z_CDW_EGI.sh 
    n=0
	#!
	#! ###########################################################
	#! begin:   Apple red/grn    Red(stroke) Grn(fill)
	#!
	#! 0 ###########################################################
	#! "EmGr" # One Single Master Embedded Graphic Image  for  all <button> 
	touch '${lcEmGrImgFnTrgTxt}'
	#!
	#! list of EmGr-img-apple-? names:
	#! redyel, grnyel, bluyel
	#! 
	#!
	#!### SHORT FORM 1 #######################################################
	### style="fill:none; fill:#00CC00; stroke:#AE3333
	### style="fill:yellow;stroke:red
	lcEmGrImgSrcRed="/tmp/EmGr-img-apple-redyel.svg"
    lcEmGrImgTrg="/tmp/EmGr-img-hash-red.xpm"
	echo "## Apple red/yel ################### " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	...
	' > ${lcEmGrImgSrcRed}
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
	cat ${lcEmGrImgTrg} >> ${gcEmGaImgFnTrgTxtMaster} 	# for Trg for "Dump" to screen
	#!
	#!### SHORT FORM 2 #######################################################
	### style="fill:none; fill:#00CC00; stroke:#AE3333
	### style="fill:yellow;stroke:green
	lcEmGrImgSrcGrn="/tmp/EmGr-img-apple-grnyel.svg"
	lcEmGrImgTrg="/tmp/EmGr-img-hash-grn.xpm"
	echo "## Apple grnyel ################### " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	echo '<?xml version="1.0" encoding="UTF-8"?>
	<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="100" width="100">
	  <g
	style="fill:yellow;stroke:green; stroke-width:12px; stroke-linecap:round; stroke-linejoin:round;">
		<path d="m 50,10 0,35"/>
		<path d="M 26,20 C -3,48 16,90 51,90 79,90 89,67 89,52 89,37 81,26 74,20"/>
	  </g>
	</svg>
	' > ${lcEmGrImgSrcGrn}
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
	cat ${lcEmGrImgTrg} >> ${gcEmGaImgFnTrgTxtMaster} 	# for Trg for "Dump" to screen
	#!
	#!### SHORT FORM 3 #######################################################
	### style="fill:none; fill:#00CC00; stroke:#AE3333
	### style="fill:yellow;stroke:blue
	lcEmGrImgSrcBlu="/tmp/EmGr-img-apple-bluyel.svg"
	lcEmGrImgTrg="/tmp/EmGr-img-hash-blu.xpm"
	echo "## Apple bluyel ################### " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	echo '<?xml version="1.0" encoding="UTF-8"?>
	<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="100" width="100">
	  <g
	style="fill:yellow;stroke:blue; stroke-width:12px; stroke-linecap:round; stroke-linejoin:round;">
		<path d="m 50,10 0,35"/>
		<path d="M 26,20 C -3,48 16,90 51,90 79,90 89,67 89,52 89,37 81,26 74,20"/>
	  </g>
	</svg>
	' > ${lcEmGrImgSrcBlu}
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
	cat ${lcEmGrImgTrg} >> ${gcEmGaImgFnTrgTxtMaster} 	# for Trg for "Dump" to screen
	#!
	#!### SHORT FORM 4 #######################################################
	### style="fill:none; fill:#00CC00; stroke:#AE3333
	### style="fill:yellow;stroke:red
	lcEmGrImgSrcYel="/tmp/EmGr-img-apple-yelred.svg"
	lcEmGrImgTrg="/tmp/EmGr-img-hash-red.xpm"
	echo "## Apple yelred ################### " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
	echo '<?xml version="1.0" encoding="UTF-8"?>
	<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="100" width="100">
	  <g
	...
	' > ${lcEmGrImgSrcYel}
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
	cat ${lcEmGrImgTrg} >> ${gcEmGaImgFnTrgTxtMaster} 	# for Trg for "Dump" to screen
	lcEmGrImgTrg="/tmp/EmGr-img-hash-grn.xpm"
	lcEmGrImgSrc="/tmp/EmGr-img-apple-redyel.svg"
	#!### SHORT FORM #######################################################
	#!### replaces hash with apple
	#!
	#!### SHORT FORM ###
	#! 1 "EmGr" # HASH #####################################################
	lcEmGrImgTrg="/tmp/EmGr-img-hash-grn.xpm"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
	cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster} 
	#!
	#!### SHORT FORM ###
	#! 2 "EmGr" ############################################################
	lcEmGrImgTrg="/tmp/EmGr-img-hash-red.xpm"
	...
	#!### SHORT FORM ###
	#! 4 "EmGr" ############################################################
	lcEmGrImgTrg="/tmp/EmGr-img-hash-nongrn.xpm"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 5 "EmGr" ############################################################
	lcEmGrImgTrg="/tmp/EmGr-img-hash-nonblu.xpm"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 6 "EmGr" ############################################################
	lcEmGrImgTrg="/tmp/EmGr-img-hash-nonred.xpm"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 7 "EmGr" # APPLE ####################################################
	lcEmGrImgTrg="/tmp/EmGr-img-apple-redred.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 8 "EmGr" ############################################################
	lcEmGrImgTrg="/tmp/EmGr-img-apple-redblu.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
 	##
	lcEmGrImgTrg="/tmp/EmGr-img-apple-redXyel.svg"
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 10 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-apple-redgrn.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 11 "EmGr"# QUIT ######################################################
	lcEmGrImgTrg="/tmp/EmGr-img-quit-rednnn.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 12 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-quit-grnnnn.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 13 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-quit-yelnnn.svg"
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
    #!
	#!### SHORT FORM ###
	#! replace jug with apple
	#! 14 "EmGr" # JUG ######################################################
	lcEmGrImgTrg="/tmp/EmGr-img-jug-bluxyel.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 15 "EmGr" ############################################################
	lcEmGrImgTrg="/tmp/EmGr-img-jug-grnxorange.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 16 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-jug-redxyel.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 17 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-jug-redxgrn.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 18 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-jug-grnxred.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	...
	#!### SHORT FORM ###
	#! 20 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-jug-redxblu.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#! replace check with apple
	#!### SHORT FORM ###
	#! 21 "EmGr" # CHECK ###################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-redXgrn.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	...
	#!### SHORT FORM ###
	#! 22 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-yelXred.svg"
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 23 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-yelXblu.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 24 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-bluXred.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 25 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-bluXgrn.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 26 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-grnXred.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 27 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-redXred.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 28 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-grnXgrn.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 29 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-yelXyel.svg"
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 30 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-bluXblu.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 31 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-redred.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 32 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-grngrn.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 33 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-yelyel.svg"
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 34 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-Check-blublu.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
    #!
	#!### SHORT FORM ###
	#! replace rect with apple
	#! 35 "EmGr" # RECT ####################################################
	lcEmGrImgTrg="/tmp/EmGr-img-RectRed.svg"
	cp ${lcEmGrImgSrcRed}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 36 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-RectGrn.svg"
	cp ${lcEmGrImgSrcGrn}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 37 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-RectYel.svg"
	cp ${lcEmGrImgSrcYel}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
	#!### SHORT FORM ###
	#! 38 "EmGr" ###########################################################
	lcEmGrImgTrg="/tmp/EmGr-img-RectBlu.svg"
	cp ${lcEmGrImgSrcBlu}        ${lcEmGrImgTrg}		# create one new Trg
		cat ${lcEmGrImgTrg} >>   ${gcEmGaImgFnTrgTxtMaster}
	#!
} # end: Sbr_CDW_EGI_Short 
#! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
export Sbr_CDW_EGI_Short 
#!
#!
#!
#!
...
#! ##########################################################################
				...
				#!
				#! 2 "EmGr"###########################################################
				lcEmGrImgSrc="/tmp/EmGr-img-hash-red.xpm"
				echo "################################## " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo '
				/* XPM */
				...
				#!
				#! 4 "EmGr"###########################################################
				lcEmGrImgSrc="/tmp/EmGr-img-hash-nongrn.xpm"
				echo "################################## " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo '
				/* XPM */
				...
				#!
				#! 5 "EmGr"###########################################################
				lcEmGrImgSrc="/tmp/EmGr-img-hash-nonblu.xpm"
				echo "################################## " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo '
				/* XPM */
				...
				#!
				#! 6 "EmGr"###########################################################
				lcEmGrImgSrc="/tmp/EmGr-img-hash-nonred.xpm"
				echo "################################## " >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo "${lcEmGrImg}" >> ${gcEmGaImgFnTrgTxtMaster}   # title for "Dump" to screen
				echo '
				/* XPM */
...
#!
#! ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#!  CheckIF : SetUp for FirstRun of "Fx(Sbr_CDW_Embedded_Graphic_Images)"
#!
...
#!
#! ############################################################
#! window DC_NoShow /tmp/EmGr-img-jug-grnxred.svg  # TEST <window colors
...
#!
#! ############################################################
#! window DC_0 # to make this the primary "small" "Double-Click" msg window. 
msg_DC_0_window='
...
#! window DC_S 
msg_DC_S_window='
<window title="msg_DC_SAMPLE_window"
window-position="nearmouse"
visible="true" >
	<frame>
		<hbox>
			<button> <label>"Clear_the_SAMPLE_Window_NOW  !  "</label>
				<input file>"/tmp/EmGr-img-hash-blu.xpm"</input><height>48></height> <width>48</width>
				<action function="closewindow">msg_DC_S_window</action>
			</button>
		</hbox>
	</frame>
	<variable>msg_DC_S_window</variable>
</window>'
export msg_DC_S_window
#!
#! ############################################################
#! window DC_ReStart_0 
...
#! ############################################################
#! window DC_ReBoot /tmp/EmGr-img-EmGr-hash-blu.xpm    S2F , E3R+CTB
export gcMsgWindowReBoot1="The ReBoot function is active now ! "
export gcMsgWindowReBoot2="Click the ReBoot Button again ! "
#!
msg_DC_ReBoot_window='
<window title="msg_Chk_ReBoot_window"
visible="true" >
	<frame>
		<text editable="false" use-markup="true" xalign=".5">
...
#!
#! ############################################################
#! window DC_ReQuit  //tmp/EmGr-img-EmGr-hash-red.xpm    S2F , E3R+CTB
export gcMsgWindowReQuit1=" the ReQuit function is active now ! "
export gcMsgWindowReQuit2=" Click the ReQuit Button again !"
#!
msg_DC_ReQuit_window='
<window title="msg_Chk_ReQuit_window"
visible="true" >
	<frame>
		<text editable="false" use-markup="true" xalign=".5">
			<label> "<span color='"'red'"'> <b>'${gcMsgWindowReQuit1}'</b> </span> " </label>
		</text>
		<text editable="false" use-markup="true" xalign=".5">
			<label> "<span color='"'green'"'> <b>'${gcMsgWindowReQuit2}'</b> </span> " </label>
		</text>
		<hbox>
			<button> <label>"Clear the Set-Up "</label>
				<input file>"/tmp/EmGr-img-hash-red.xpm"</input><height>48></height> <width>48</width>
				<action function="closewindow">msg_DC_ReQuit_window</action>
			</button>
		</hbox>
	</frame>
	<variable>msg_DC_ReQuit_window</variable>
</window>'
export msg_DC_ReQuit_window
#!
#! ### Not Used #########################################################
#! window DC_PowerOff  //tmp/EmGr-img-EmGr-hash-red.xpm    S2F , E3R+CTB
msg_DC_PowerOff_window='
<window title="msg_DzC_PowerOff_window"
window-position="nearmouse"
visible="true" >
	<frame>
		<text editable="false" use-markup="true" xalign=".5">
...
export gcMsgWindowPowerOff2=" Click the PowerOff Button again !"
#!
#!
#! ### Not Used #########################################################
#! window DC_ProgramOff //tmp/EmGr-img-EmGr-hash-red.xpm    S2F , E3R+CTB
msg_DC_ProgramOff_window='
<window title="msg_DxC_ProgramOff_window"
window-position="nearmouse"
visible="true" >
	<frame>
		<text editable="false" use-markup="true" xalign=".5">
		...
		<hbox>
			<button> <label>"Clear the Set-Up "</label>
				<input file>"/tmp/EmGr-img-hash-red.xpm"</input><height>48></height> <width>48</width>
				<action function="closewindow">msg_DC_ProgramOff_window</action>
			</button>
		</hbox>
	</frame>
	<variable>msg_DC_ProgramOff_window</variable>
</window>'
export msg_DC_ProgramOff_window
export gcMsgWindowProgramOff1=" the ProgramOff function is active now ! "
...
#!
#! ############################################################
#! window DC_E3CTARS function /tmp/EmGr-img-EmGr-hash-grn.xpm   S2F , E3R+CTB+ReStart
export gcMsgE3rCtarsWindowMsg1="The E3CTARS function is active now ! "
export gcMsgE3rCtarsWindowMsg2="     E3R + ReStart ! "
export gcMsgE3rCtarsWindowMsg3="Click the System Button again ! "
#!
msg_DC_guiE3RctarS_window='
<window title="msg_E3CTARS_window"
visible="true" ><frame>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'red'"'>   <b>'${gcMsgE3rCtarsWindowMsg1}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rCtarsWindowMsg2}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rCtarsWindowMsg3}'</b> </span> " </label></text>
<hbox><button>
<label>"Clear the Set-Up "</label><input file>"/tmp/EmGr-img-hash-grn.xpm"</input><height>48></height> <width>48</width>
<action function="closewindow">msg_DC_guiE3RctarS_window</action>
</button></hbox></frame>
<variable>msg_DC_guiE3RctarS_window</variable>
</window>'
export msg_DC_guiE3RctarS_window
# export msg_DC_guiE3RctarS_window
#!
#!
#! ############################################################
#! window DC_E3CTARB function /tmp/EmGr-img-EmGr-hash-grn.xpm   S2F , E3R+CTB+ReStart
export gcMsgE3rCtarbWindowMsg1="The E3CTARB function is active now ! "
export gcMsgE3rCtarbWindowMsg2="     E3R + ReBoot ! "
export gcMsgE3rCtarbWindowMsg3="Click the System Button again ! "
#!
msg_DC_guiE3RCTARB_window='
<window title="msg_E3CTARB_window"
visible="true" ><frame>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'red'"'>   <b>'${gcMsgE3rCtarbWindowMsg1}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rCtarbWindowMsg2}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rCtarbWindowMsg3}'</b> </span> " </label></text>
<hbox><button>
<label>"Clear the Set-Up "</label><input file>"/tmp/EmGr-img-hash-grn.xpm"</input><height>48></height> <width>48</width>
<action function="closewindow">msg_DC_guiE3RCTARB_window</action>
</button></hbox></frame>
<variable>msg_DC_guiE3RCTARB_window</variable>
</window>'
export msg_DC_guiE3RCTARB_window
#! export msg_DC_guiE3RCTARB_window
#!
#!
#! ############################################################
#! window DC_E3CTARQ function /tmp/EmGr-img-EmGr-hash-grn.xpm   S2F , E3R+CTB+ReQuit
export gcMsgE3rCtarqWindowMsg1="The E3CTARQ function is active now ! "
export gcMsgE3rCtarqWindowMsg2="     E3R + ReQuit ! "
export gcMsgE3rCtarqWindowMsg3="Click the System Button again ! "
#!
msg_DC_guiE3RCTARQ_window='
<window title="msg_E3CTARQ_window"
visible="true" ><frame>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'red'"'>   <b>'${gcMsgE3rCtarqWindowMsg1}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rCtarqWindowMsg2}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rCtarqWindowMsg3}'</b> </span> " </label></text>
<hbox><button>
<label>"Clear the Set-Up "</label><input file>"/tmp/EmGr-img-hash-grn.xpm"</input><height>48></height> <width>48</width>
<action function="closewindow">msg_DC_guiE3RCTARQ_window</action>
</button></hbox></frame>
<variable>msg_DC_guiE3RCTARQ_window</variable>
</window>'
export msg_DC_guiE3RCTARQ_window
#! export msg_DC_guiE3RCTARQ_window
#!
#!
#!
...
#! ### all like _K-K_  #########################################################
#! window DC_E3C_M_B 
export gcMsgE3rE3C_BM_RB_WindowMsg1="The E3R_Build-Menu_ReBoot function is active now ! "
export gcMsgE3rE3C_BM_RB_WindowMsg2="    E3R + Build-Menu_Boot ! "
export gcMsgE3rE3C_BM_RB_WindowMsg3="Click the System Button again ! "
#!
msg_DC_E3R_BM_RB_window='
<window title="msg_E3R_Build-Menu_ReBoot_window"
visible="true" ><frame>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'red'"'>   <b>'${gcMsgE3rE3C_BM_RB_WindowMsg1}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rE3C_BM_RB_WindowMsg2}'</b> </span> " </label></text>
<text editable="false" use-markup="true" xalign=".5"><label> "<span color='"'green'"'> <b>'${gcMsgE3rE3C_BM_RB_WindowMsg3}'</b> </span> " </label></text>
<hbox><button>
<label>" Clear the Set-Up "</label><input file>"/tmp/EmGr-img-hash-grn.xpm"</input><height>48></height> <width>48</width>
<action function="closewindow">msg_DC_E3R_BM_RB_window</action>
</button></hbox></frame>
<variable>msg_DC_E3R_BM_RB_window</variable>
</window>'
export msg_DC_E3R_BM_RB_window
#!
#!
#!
#!
...
#(:
#! ############################################################
function Sbr_Cbx_ReQuit() {   # Active Check-Box # ARB_ReQuit
	n=0
	#
	if [ $1 ] ; then
...
function Sbr_SDA_Only() {    # Active  if[!SDA] then restart
    ...
    if [ ! -f "/initrd/mnt/dev_save/MARK-SDA" ] || [ ! -f "/initrd/mnt/dev_save/MARK-SDA1" ]; then  
        ...
        #!  fail = ReStart
        Sbr_Cbx_ReStart 1   ### do not continue the called command.
    fi
...
#(: purpose: just a handy splash for Column 4  guiS2F-ReStart, ReBoot, PowerOff
...
#!
            #~ #(: =======================================================================
            #~ #(: fx(Sbr_CpyRootConfig)   Not Called 
            #~ #(: called by:  EXIT:guiCpyRootConfig     Not Called 
            #~ #(: param:
            #~ #(: purpose:
            #~ #(: calls: 
            #~ #(: method:
            ...
            #~ #(:
            #~ #! ### Sbr_CpyRootConfig #########################################################
            #~ function Sbr_CpyRootConfig() {		  ###  Not USED 
            #~ #------------------------------------------------------------------------------
            #~ #------------------------------------------------------------------------------
            #~ lcSrcParam1=$1
            #~ lcTrgParam2=$2
            #~ #! -D----------------------------gtkdialog--
            #~ /usr/lib/gtkdialog/box_splash -fg red -bg yellow -border true  -close never -text   "RootConfig: ${lcTrgParam2}  " &
            #~ GTKPIDguiSetUp1=$!	### kill #((GTKPIDguiSetUp
            #~ cp -v -r  ${lcSrcParam1}  ${lcTrgParam2} 
            #~ #! -D----------------------------gtkdialog--
                #~ kill $GTKPIDRootConfig1
            #~ } # end: Sbr_CpyRootConfig
            #~ export Sbr_CpyRootConfig
#!
#!
#!
#(: ====================================================================
#(:                    ===================== 
#(: begin: CTB Sbr Box DATA Dump SubRoutines : syncronized MGR : CTX 
#(:                    =====================   
#(: ====================================================================
...
# ACTIVE   for HELP and FAQ
                ...
                #!
                #(: ===========================================================
                #(: fx(Sbr_Box_Splash)	# internal   ### NOT ACTIVE
                #(: called by : tested by Bugger_BoxSplash
                #(: call method: Sbr_Box_Splash text fg bg
                #(: purpose :  Simple Splash calling "gtkdialog/box_splash"
                #(: receives param : text string, color, color
                #(: returns : with pid to kill
                #(: method: gtkdialog box_splash req string
                #(: note: todo: embed code for "/usr/lib/gtkdialog/box_splash"
                #(: note:
                ...
                #(:
                #! ############################################################
                #! function Sbr_Box_Test() {  ### NOT ACTIVE
                #! n=0
                #(:  BasePIDfile is "/tmp/box_help"
                #(:  output storage file MUST BE "/tmp/box_help"
                #(:  due to hard-code in "/usr/lib/gtkdialog/box_help"
                #(:  Modifying would create a "dependency" problem for users.
                #! } # end: Sbr_Box_Test   ### NOT ACTIVE
                #!
#!
#(: ============================================================
#(: fx(Sbr_ARB_Init)	#: Active #   menulst   menu.lst   build
#(: called by:   main run ( )
...
function Sbr_Multi_Button_01() {  # ACTIVE
	n=0
#! ????? Sbr_Multi_Button
lcMsgStr=">>> Sys_Sample_Button_Dialog NOT ACTIVE >>>" && Sbr_Str2DevTrm "$lcMsgStr" "7" "3"
...
<vbox>
	...
	<hbox>
		...
		<button> <label>"Multi_Button ACTIVE " </label>
            <input file>"'/tmp/EmGr-img-jug-redxyel.svg'"</input><width>15</width><height>15</height>   
            <action>echo : You pressed the button with the 'Multi_Button' label </action>
		</button>
		<button> <input file>"/tmp/EmGr-img-hash-red.xpm"</input>
		  <action>echo action: You pressed the button with the pixmap.</action>
		</button>
		<button yes>  	<action>echo action: You pressed the Yes button.</action> 	</button>
		<button no>   	<action>echo action: You pressed the No button. </action> 	</button>
		<button ok>   	<action>echo action: You pressed the OK button.</action>  	</button>
		<button help>   <action>echo action: You pressed the Help button.</action>  </button>
...
#~ /window>  
	...
	#~ <vbox>
		#~ <button>
		  #~ <label>"Multi_Button ACTIVE " </label>
		  #~ <action>echo : You pressed the button with the 'SB' label </action>
		#~ </button>
		#~ <button>
...
<span  foreground='"'red'"'>"iso-KineticPup32-22.10+2_______________230604"</span>
<span  foreground='"'red'"'>"iso-BionicPup32-8.0+30_K4.9____________220407"</span>
<span foreground='"'blue'"'>"iso_BionicPup32-19.03__K5.9____________231209"</span>
<span foreground='"'blue'"'>"iso_ManticPup32-23.10+1________________231209"</span>
<span  foreground='"'red'"'>"iso-JammyPup32-22.04+8_________________22123"</span>
<span  foreground='"'red'"'>"iso-UPupFF+D-20.12+4_Focal-Fossa_______201210"</span>
...
<span  foreground='"'red'"'>"iso-UpupTahr-6.0.6-K4.1.30_____________190211"</span>
<span foreground='"'green'"'>"__________________________________________"</span>
"--- "
...
#!
#~ #(: Base-64 
#~ #(: now, call the Convert-64 routine for output file. 
#! ############################################################
#~ function Sbr_Base64() {  #: ACTIVE Col 3
    #~ n=0 # ALWAYS install a stub, so 'funxtion' is legit code
#~ lcBase64fn="/tmp/box_help" # "${lcMsgTrg}"  ### "${0}"
#~ lcBase64fnEncoded="/tmp/Base64-Encoded.txt"
#~ #!
#~ base64 ${lcBase64fn} > ${lcBase64fnEncoded}
#~ gxmessage -title "Base64"  -name "-B64-" -font bold -bg lightyellow -fg darkblue -center -wrap -file ${lcBase64fnEncoded}
#~ #!
#~ echo "" && echo "=== Base64  cat ${lcBase64fnEncoded} ============read "
#~ echo "" && echo "=== Base64  cat /base-64-decoded.txt ===================="
#~ #!
#~ lcBase64fnDecoded="/tmp/Base-64-Decoded.txt"
#~ base64 -d ${lcBase64fnEncoded} > ${lcBase64fnDecoded}
#~ gxmessage -title "Base64"  -name "-B64-" -font bold -bg lightyellow -fg darkblue -center -wrap -file ${lcBase64fnDecoded}
#!
} # end: Sbr_Dump_SRC
...
#(: called by:    EXIT:guiS2F-ReStart, ReBoot, ReQuit
#(: param:
#(: purpose:
...
function Sbr_Menu_Standard_Selection_Header() { 	# Standard Selection Header 
		...
		echo "title  1 { K-K }---UpupKK+D-22.  -------K:5------------_${gcBootOsHd} \n   \n   \n"
		...
		echo "title  2 { B5B }---UpupBB-K5 ---Edit----K:5---FF--LO--- ${gcBootOsHd}  \n   \n   \n"
		echo "  find --set-root --ignore-floppies --ignore-cd   /MARK-${gcBootOsHd}"
		...
		echo "title  3 { J-J }---UpupJJ+D+22.04 L6 ---K:5------------ ${gcBootOsHd} \n   \n   \n"
		echo "  find --set-root --ignore-floppies --ignore-cd   /MARK-${gcBootOsHd}"
	    ...
	    echo "# #####################################{ S15Slacko-64 }"
		echo "title  9 { S64 }---SpupSlacko64           --------------${gcBootOsHd} \n   \n   \n"
		echo "  find --set-root --ignore-floppies --ignore-cd   /MARK-${gcBootOsHd}"
		...
		echo "title  -10- { M-M }-UpupMM-23.11 Mantic-Minotaur-L6-K ---${gcBootOsHd} \n   \n   \n"
		echo "  find --set-root --ignore-floppies --ignore-cd    /MARK-${gcBootOsHd}"
		...
		echo "# #######################{ Slacko-LxSc }"
		echo "title  -14- { SLxSc }- Pup_Slacko-LxSc-20.01  -----------${gcBootOsHd} \n  \n  \n"
		echo "  find --set-root --ignore-floppies --ignore-cd    /MARK-${gcBootOsHd}"
		...
		echo "###################################{ Bionic Beaver  K:4  }"
		echo "title  -17- { B4B }--- UpupBB-K4 ------------------------${gcBootOsHd}  \n   \n   \n"
		echo "  find --set-root --ignore-floppies --ignore-cd   /MARK-${gcBootOsHd}"
...
echo " "
echo " "
Sbr_CDW_Embedded_Dialog_Colors		# main ()
Sbr_CDW_EGI_CheckIF		# main () Setup for Short vs Long Image format.
#!
#;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
#!
Sbr_ARB_Init   #: ARB_00:  Main Run ( ) calls fx(Sbr_ARB_Init) : read config & write menu.lst
#!
#;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
#!
...
#!
#! problem: each time the Main Gui is run,
#!    there is a possible delay ??? of several seconds 
#!    until Main Gui has repositioned the <button> .
#!    until Main Gui <button> are 'active'.
#!    may be dependent on re-sizing each element of gui.
#!    may be dependent on graphical code of each OS. 
#!
#! cause: each OS has special display pixel count, font size, etc.
#!
#! to do: continue :
#! run gui test on each OS:__________Menu # __Distro Sub Name
#! GTK gui is dependent on system each "theme" and "font" size. 
#!
######################################################################
#! run gui test on each OS:__________Menu # __Distro Sub Name
#! export lcGuiWidthRequest="1730"		# 2 "upupKK" Raleigh theme
...
#~ export lcGuiWindowTitle="___MGR_${gcSysDistroLoc}:-:${gcSysSave}:-:${gcSysIdFull}:-:${gcSysKernName}:-:${gcSysDevHw}:-:${gcSysDevDob}:-:${gcMgrVer}"
export lcGuiWindowTitle="___MGS_${gcSysDistroLoc}:-:${lcGuiSplT0}:-:_Dob:${gcSysDevDob}_${gcSysSave}:-:${lcGuiSplT1}:"
#! ${gcMgrDistroName}
...
export lcGuiFrameTitle="${gcMgrDevName}_"
#!

#!
export lcToolTip="E3R + Build Menu.lst + ReBoot ?"
#!
#! ## MAINGUI # MGS # Begin ##################################################
#!

export MGS_Master_GUI='
<window  
...
<vbox>
	...
	<vbox>
      <button  height-request="39" tooltip-text="Frame-Title Manager+ReStart
       This is LINUX,
	...
	<vseparator height-request="3"> </vseparator>
		...
		<vbox>
        <hbox>
			<button visible="0" label=""  tooltip-text="* Before Title *"
			has-focus="false" height-request="1" width-request="35" space-expand="false" >
			<action>lxAction:* Before Title *</action> </button>
        </hbox>
		</vbox>

		<hbox>

<button tooltip-text="'Margin-C0vert'" width-request="1"> <action>lxAction:mC0</action> </button>
<button tooltip-text="'Margin-C0vert'" width-request="1"> <action>lxAction:mC0</action> </button>
<button tooltip-text="'Margin-C0vert'" width-request="1"> <action>lxAction:mC0</action> </button>
<button tooltip-text="'Margin-C0vert'" width-request="1"> <action>lxAction:mC0</action> </button>
<button tooltip-text="'Margin-C0vert'" width-request="1"> <action>lxAction:mC0</action> </button>
...
<vbox>
            <button visible="true" has-focus="false" height-request="30"  xalign=".5"  tooltip-text="===C4===" >
            ...
            <text visible="true" editable="false" use-markup="true" xalign=".5"> <label>
           "<span  color='"'green'"' font-family='"'Comic'"' weight='"'bold'"' > <b>'"ooo-"'</b> </span><span color='"'red'"' font-family='"'Comic'"' weight='"'bold'"' size='"'xx-large'"' ><b>'"MGR_S1"'</b></span><span color='"'blue'"' font-family='"'Comic'"' weight='"'bold'"' >'-ooo'</span>"
           </label> </text>
    ...
    <hbox homogeneous="true">
       ...
       <button has-focus="false" height-request="15"  xalign=".5"  tooltip-text="C4-/tmp/EmGr-img-apple-redblu.svg" >
        ...
        <button visible="false" has-focus="false" height-request="3"  xalign=".5"  tooltip-text="Top-C4a" >
            <label> "'---*---'" </label> <action>lxAction:Top-C4pre</action></button>
        <button visible="true" has-focus="false" height-request="3"  xalign=".5"  tooltip-text="Top-C4b" >
            <label> "'---*---'" </label> <action>lxAction:Top-C4pre</action></button>
		<vseparator height-request="3"> </vseparator>

        <text editable="false" use-markup="true" xalign="0.5">
           <label> "<span color='"'$lcGuiColorblu'"'><b>{'_PreSet__1-of-17_PuppyOS__menu.lst_'}</b></span>"</label> 
           </text>

    <hbox homogeneous="true">
            <hseparator width-request="10"></hseparator>

            <button visible="false" has-focus="false" height-request="15"  xalign="1">
                <input file>"'/tmp/EmGr-img-hash-grn.xpm'"</input><width>15</width><height>15</height>
                <label>"''"</label><action>'left-alt'</action> </button>

            <button has-focus="false" height-request="30"  xalign=".5"
                tooltip-text="'Generic_Menu.lst_Yellow/Blue/Yellow/Red_'
             Build Menu.lst and ReStart   ">
                <input file>"'/tmp/EmGr-img-jug-grnxred.svg'"</input><width>15</width><height>15</height>
                <label>"'ReMenu=Yel/Blu'"</label>
               <action function="'launch'">msg_DC_ReMenu_window</action>
               <action function="'closewindow'">msg_DC_ReMenu_window</action>
                <action>'EXIT:ARB_ORGYBYR'</action> </button>

                        <button visible="false" has-focus="false" height-request="30"  xalign=".5"
            ...
            <button  visible="false" has-focus="false" height-request="15"  xalign="0">
                <input file>"'/tmp/EmGr-img-hash-grn.xpm'"</input><width>15</width><height>15</height>
                <label>"''"</label><action>'right-alt'</action> </button>
...
<vbox>
            ...
            <text editable="false" use-markup="true" xalign="0.5">
                <label> "<span color='"'$lcGuiColorgrn'"'><b>{'_Write_Menu.lst_+_ReBoot__'}</b></span>"</label> 
                </text>
    <hbox homogeneous="false">
     ...
     <hseparator width-request="12"></hseparator>
           ...
           <button has-focus="false" height-request="30"  xalign=".5">
                <input file>"'/tmp/EmGr-img-hash-grn.xpm'"</input><width>15</width><height>15</height>
                <label>"''"</label><action>'right-specific-menu-options'</action> </button>
            <hseparator width-request="97"></hseparator>
    </hbox>
    <hbox homogeneous="false">
            <hseparator width-request="70"></hseparator>
           <button has-focus="false" height-request="30"  xalign=".5">
                <input file>"'/tmp/EmGr-img-hash-grn.xpm'"</input><width>15</width><height>15</height>
                <label>"''"</label><action>'left-smo'</action> </button>
            <button label="'_T-6__'"     tooltip-text="'[]_Tahr_EXIT:ARB_T-6'
                Build Menu.lst + ReBoot  " has-focus="false" height-request="25">
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
                    <action>'EXIT:ARB_T6'</action>  </button>
            <button label="'_S-7__'"    tooltip-text="'[]_Slacko7-Alternate_EXIT:ARB_S-7'
                Build Menu.lst + ReBoot  " has-focus="false" height-request="25">
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
                    <action>'EXIT:ARB_S7'</action>  </button>
			<button label="'_uIMP__'"    tooltip-text="'[]_UpupIMP_EXIT:ARB_UIMP'
                Build Menu.lst + ReBoot  " has-focus="false" height-request="25">
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
					<action>'EXIT:ARB_UIMP'</action>  </button>
            <button label="'_S64__'"    tooltip-text="'[]_Slack64-Kern:4_EXIT:ARB_S64'
                Build Menu.lst + ReBoot  " has-focus="false" height-request="25">
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
     ...
     <hseparator width-request="11"></hseparator>
           ...
           <button has-focus="false" height-request="30"  xalign=".5">
            ...
            <button label="'_SLXSC_'"  tooltip-text="'[]_Slacko-LxSc-Kern:4_EXIT:ARB_SLXSC'
                Build Menu.lst + ReBoot  " has-focus="false" height-request="25">
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
    ...
    <vbox>
        ...
        <text editable="false" use-markup="true" xalign="0.5">
                <label> "<span color='"'$lcGuiColorora'"' size='"'small'"'><b>'======================'</b></span>"</label></text>

		<hbox homogeneous="true">
        <text visible="true" editable="false" use-markup="true" xalign=".5"> <label>
           "<span  color='"'$lcGuiColorgrn'"' font-family='"'Comic'"' weight='"'bold'"' size='"'medium'"'> <b>'"{-"'</b> </span>   <span color='"'$lcGuiColorblu'"' font-family='"'Comic'"' weight='"'bold'"' size='"'large'"'><b>'"Double-Click"'</b></span>   <span color='"'$lcGuiColorgrn'"' font-family='"'Comic'"' weight='"'bold'"' size='"'medium'"'>'-}'</span>"</label> </text>
        </hbox>
        ...
        <text visible="false" editable="false" use-markup="true" xalign="0.5">
                <label> "<span color='"'$lcGuiColorgrn'"' size='"'medium'"'><b>{'_+++_Double-Click-+++_'}</b></span>"</label></text>

		<hbox homogeneous="true">
		<button label="'===-E3R-Tools-==='"
            visible="true" 		has-focus="false" height-request="53" use-stock="true"
            tooltip-text="
	...
	ArcDts +E3R +S2F +pMnt +CTB +uMnt,   .
	+ 'PowerOff' system {!}">
			<input file>"/tmp/EmGr-img-jug-redxgrn.svg"</input><width>27</width><height>27</height>
			<label> 'E3R+CTB+PowerOff' </label>
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
				<action>'EXIT:guiE3RCTARQ'</action>
            </button>
            <hseparator width-request="80"></hseparator>
		</hbox>
    </vbox>
...
<button visible="false" tooltip-text="##################################################" ><label>""</label><action>'EXIT:xxx'</action></button>
	...
	<vbox>
    	...
    	<button tooltip-text="before CTX" height-request="1" has-focus="false"> 
            <input file icon="gtk-select-color" ></input> <action>lxAction:0</action> </button>
	...
	<hbox homogeneous="true">
		...
		<button  visible="1" 
                ...
                <label> "{'Arc+DTS'}"</label>
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
                <action>'EXIT:guiAllArcDts'</action> </button>
        <button has-focus="false" height-request="30"  xalign=".5"
                tooltip-text="'EXIT:guiCTX'< 'CTX' generic from Local to SDA1 Drive, has GUI >">
                <input file>"'/tmp/EmGr-img-jug-grnxred.svg'"</input><width>25</width><height>25</height>
                <label>"'{C.T.B.}'"</label>
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
				<action>'EXIT:guiCTX'</action>  </button>
        <button has-focus="false" height-request="30"  xalign=".5"
                tooltip-text="'EXIT:guiReMGR'< 'MGR' master program, has Master GUI >">
                <input file>"'/tmp/EmGr-img-jug-redxyel.svg'"</input><width>25</width><height>25</height>
                <label>"'{MGR}'"</label>
                <action function="'launch'">msg_DC_NoShow_window</action><action function="'closewindow'">msg_DC_NoShow_window</action>
				<action>'EXIT:guiReMGR'</action>  </button>
           <hseparator width-request="1"></hseparator>
    </hbox>

            <hseparator width-request="0"></hseparator>

		<button tooltip-text="after CTX" height-request="1" has-focus="false"> 
            <input file icon="gtk-select-color" ></input> <action>lxAction:0</action> </button>
        ...
        <hbox homogeneous="true">
           <hseparator width-request="0"></hseparator>
            ...
            <button tooltip-text="_guiSfs-PowerOff_">
                ...
                <action function="'launch'"     >msg_DC_ReQuit_window</action>
                <action function="'closewindow'">msg_DC_ReQuit_window</action>
...
<vseparator  tooltip-text="* vseparator is Not-VISIBLE *" label="Not Showing Text" height-request="0"></vseparator>
		<vseparator height-request="1" > </vseparator>
        <button has-focus="false" height-request="19"  xalign=".5"  tooltip-text="C4-/tmp/EmGr-img-apple-redblu.svg" >
			<input file>"'/tmp/EmGr-img-apple-redgrn.svg'"</input><width>25</width>  <height>25</height>
            <label> " " </label><action>lxAction:C4</action></button>
	</vbox>

        <button tooltip-text="'Margin-C9vertical'" width-request="1"> <action>lxAction:mC0</action> </button>
        <button tooltip-text="'Margin-C9vertical'" width-request="1"> <action>lxAction:mC0</action> </button>
        <button tooltip-text="'Margin-C9vertical'" width-request="1"> <action>lxAction:mC0</action> </button>
        <button tooltip-text="'Margin-C9vertical'" width-request="1"> <action>lxAction:mC0</action> </button>
        <button tooltip-text="'Margin-C9vertical'" width-request="1"> <action>lxAction:mC0</action> </button>
...
case $EXIT in
	...
	*) eval "`gtkdialog --center -p MGS_Master_GUI`" ;;
esac
...
#(: check for prior "APPLY Checks" commands
	...
	#
	Sbr_Cbx_ReStart 0  	 # no wait # CBX APPLY+ in case something fails
	#
...
.         MGR, MGS, MGT MCI, E3R, CTX   and geany.config   with DTS  
...
if [ "$EXIT" = "guiS2F" ]  ; then
	...
	lcFnRun="MGR-S2F-0"
	echo "=== running Save2Flash ${gcS2FwaitCount} loops ====="
/bin/echo -en "\\033[0;39m"		>  /dev/console
/bin/echo -en "\\033[60G"    >/dev/console
...
echo "=== '_S2F-3_' begin: While Do Loop ==================================================="
	WAITCNT="${gcS2FwaitCount}"	# MGR-S2F-0 will Loop 1,2,3 times.
...
if [ "$EXIT" = "guiReMGR" ] ; then
    ...
    if [ -f ${lcFileRunMgr} ] ; then  
        ...
        n=0
        #! -D----------------------------gtkdialog--
        /usr/lib/gtkdialog/box_ok "ERROR" error "{!!} MGR 'if ${lcFnRunIdMgr}' Not Found:"
    fi
    #
    Sbr_Cbx_ReStart 1
    #
...
#!
                ...
                #~ #(:
                #~ #! ### Not Used ###################################################################
                #~ #! function [EXIT:guiQuick-ReStart]   # Not Used
                #~ if [ "$EXIT" = "guiQuick-ReStart" ] ; then
                    #~ n=0
                    #~ Sbr_Cold_Splash "Save  &  Re-Start MGR" "1"
                    #~ #
                    #~ Sbr_ClearFlagsSystem 0  	### clear lockfile so that Warm Reboot can run.
                ...
                #~ #(:
                #~ #! ### Not Used ###################################################################
                #~ #! function [EXIT:guiQuick-ReBoot]   # Not Used 
                #~ if [ "$EXIT" = "guiQuick-ReBoot" ];then
                    #~ #
                    #~ n=0
                    #~ #
                    #~ Sbr_Cold_Splash "Save  &  Warm ReBoot" "1"
                ...
                #~ #!
                #! ### Not Used ###################################################################
                #! function [EXIT:guiQuick-PowerOff]   # Not Called 
                #~ if [ "$EXIT" = "guiQuick-PowerOff" ] ; then
                    #~ n=0
                    #~ Sbr_Cold_Splash "Save  &  Power-Off  via  WrmPowerOff" "1"
                    #~ Sbr_ClearFlagsSystem 0  	### clear lockfile so that WrmPowerOff can run.
                    #~ Sbr_Cbx_ReQuit 1   # EXIT:guiS2F-PowerOff
                    ...
                    #~ ### "exit 0' should terminate this program.
                    #~ exit 0
                #~ fi # # end: ft(EXIT:guiQuick-PowerOff)
                #(:
                #(: ====================================================================
                #(: ft(EXIT:xxxguiQuick-PowerOff)
                #(: called by: Main GUI Fall-Thru and code is INTERNAL to MGR
                #(: param:
                #(: purpose:
                #(: calls:
                ...
                #(:
                #! ### Not Used ###################################################################
                #! function [EXIT:xxx]  # Not Called 
                #~ if [ "$EXIT" = "xxx" ] || [ "$EXIT" = "Select" ]  || [ "$EXIT" = "0" ] ; then
                    #~ n=0
                    #~ Sbr_Cold_Splash "ReStart" "1"
                    #~ Sbr_Cbx_ReStart 1   
                    #~ EXIT=""	### force "Fall Thru" if "exit 0" fails.
                    #~ ### "exit 0' should terminate this program.
                    #~ exit 0
...
#(:         (2) call Sbr_ARB_00 to test Config & build menu.lst
#(: sends:
#(: rationale:
#(: note : main run ( )   calls init: fx(Sbr_ARB_Init) to fetch configfile data.
#(: C3M.sh copied into here.
#(: # ARB_00, KK, BB, JJ, MM, BW, FF, SLXSC, S7, SLXSC, Q7, etc
#---------------------- C3M
#!
#! ############################################################
### begin: Build Short Menu  ##################################
#! ############################################################
#! if [ EXIT:ARB_00 and Short-Names ]
#! setup Specified single menu 
#! function "if"    
if [ "$EXIT" = "ARB_00" ] || [ "$EXIT" = "ARB_KK" ] || [ "$EXIT" = "ARB_B4B" ] || [ "$EXIT" = "ARB_B5B" ] || [ "$EXIT" = "ARB_JJ" ] || [ "$EXIT" = "ARB_MM" ] || [ "$EXIT" = "ARB_BW" ] || [ "$EXIT" = "ARB_BC" ] || [ "$EXIT" = "ARB_FF" ]  || [ "$EXIT" = "ARB_S7" ] || [ "$EXIT" = "ARB_T6" ] || [ "$EXIT" = "ARB_X64" ] || [ "$EXIT" = "ARB_X708" ]  || [ "$EXIT" = "ARB_UIMP" ] || [ "$EXIT" = "ARB_S64" ] || [ "$EXIT" = "ARB_S1564" ]   || [ "$EXIT" = "ARB_SLXSC" ] || [ "$EXIT" = "ARB_SLXSC" ] || [ "$EXIT" = "ARB_Q7" ]  ; then
    #!
	Sbr_SDA_Only	# control: test "SDA" else  ReStart
	Sbr_KillProc  "$EXIT"   # permission control 

    #! Sbr_Menu_Standard_Selection_Header presets "defaults" 
	#! declare Preset Var. 
		...
		if [ "$EXIT" = "ARB_JJ" ] ; then
			...
			lcOsSubFn="/mnt/home/menu.lstJJ"    # Not Used
	...
	.					                     "&
		...
		echo "#=========================================="
		echo "# SHORT MENU is a Build by 'MGR'"
		echo "# 'ID = '${lcC3MOsSubIdd}'"
		echo "# used by rc.z_MGR.sh "
		echo "# insert user selected OS parameters"
		echo "# allow default forground/background colors"
		echo "#======================================================"
	...
	echo "# #######################################################"
        ...
        #
		#~ echo "###################################{ FIT }"
		#~ echo "title   { KWX1 White }- " " \_"
		#~ echo "  find --set-root --ignore-floppies --ignore-cd /MARK-KWX1X"
		#~ echo "  configfile /menu.lst"


        ### above: note peculiar end-of-line comments 
        ### adding extra quotes may cause fault  
		#
		sleep 3
...
# working E3R   IF writing any short menu, THEN should ReStart here. 

			#=============== IF writing a "Short" menu
            #=============== THEN call E3R 

# if Long List of Short Menus 

            #~ if [ "$EXIT" = "ARB_00" ] ; then   
            #! simply install a "Simple" menu.lst, then ReBoot and view menu.lst in Geany 
                n=0
                Sbr_Cbx_ReBoot  "1"
                #~ Sbr_Cbx_ReStart "1"
            #~ else    
                #~ n=0
                #~ Sbr_Cbx_ReStart  "1"       # only during dev / testing 
                #~ Sbr_E3R_Call    # all Short Menu must run E3R 
                #~ Sbr_Cbx_ReBoot  "1"
            #~ fi
			#~ Sbr_Cbx_ReStart  "1"
            #~ exit   # has already called for ReStart ! 
			#=============== now
...
#! ======================================================================
#!
#! === LONG MENU === ARB_ORGYB | ARB_ORGWG ===================
#! === LONG-MENU === LONGMENU === LONGMENU === LONGMENU ======
#! === LONGMENU ==============================================
#!
#! function [EXIT:C3M_ARB_MenuLong]() { ; n=0 ; } # # based on "C3M"
#! function [EXIT:C3M_ARB_ORG]() { ; n=0 ; } # # based on "C3M"
#! function [EXIT:ARB_ORG]() [EXIT:ARB_ORGYB]() [EXIT:ARB_ORGWG]()
#(: ft(EXIT:ARB_ORGYBYR) 
#!
#(: =======================================================
...
#(: } # end: Menu.lst
#! ===========================================================
#! function if [ EXIT:ARB_ORGYB ] 
#! generate "Original" Menu.lst default = 0
#! capture Larger Group 
if [ "$EXIT" = "ARB_ORGYBYR" ] ; then 
  	#! ###  "$EXIT" = "ARB_ORG" "ARB_ORGCBYR" "ARB_ORGGBYR" 
	Sbr_SDA_Only	# control: test "SDA" && ReStart
#! ======================================================================
#!
#! ======================================================================
...
#! function EXIT:ARB_OriginalMenu() { ; n=0 ; } # based on "C3M" # custom long "original" is generated.
    ...
    lcC3MOsSubIdd="ORG"
    lcOsSubDir="Main"
    export lcOsSubFn="/mnt/home/menu-orig.lst"
    export lcOsSubSplXpm="${lcOsSubClrBase}/xpm-c-yel.xpm"
    export lcOsSubSplClr="red"
	export lcOsSubSplClrStr="yellow/green white/blue"	# default
    ...
    #! enable Smaller Group
#~ case "${EXIT}" in   # large menu background/fg colors white/brown yellow/blue
   #~ "ARB_ORGYBYR") lcOsSubSplClrStr="yellow/blue yellow/red cyan/black light-blue/black"
					#~ lcOsSubSplClrID="Yellow/Black/Red"
					#~ lcOsSubSplClrIfg="Yellow"		# # Selection:Red/Black  Menu:Cyan/Blue
					#~ lcOsSubSplClrIbg="blue"		
					#~ ;;
   #~ "xARB_ORGGBYR") lcOsSubSplClrStr="green/black yellow/red cyan/black light-blue/black"
					#~ lcOsSubSplClrID="Green/Black/Red"
					#~ lcOsSubSplClrIfg="Yellow"	# Selection:Yellow/Green  Menu:Green/Black
					#~ lcOsSubSplClrIbg="Green"	
					#~ ;;
   #~ "xARB_ORGRYBG") lcOsSubSplClrStr="black/green red/yellow black/cyan black/light-blue"
					#~ lcOsSubSplClrID="Green/Black/Red"
					#~ lcOsSubSplClrIfg="Red"	# letters:Red/Yellow FB:Blue/Cyan
					#~ lcOsSubSplClrIbg="Yellow"	
					#~ ;;
    #~ *)				lcOsSubSplClrStr="yellow/green cyan/blue"  # default to "00"  # Wierd Color Scheme
					#~ lcOsSubSplClrID="Yellow/Green"
					#~ lcOsSubSplClrIfg="Blue"
					#~ lcOsSubSplClrIbg="Cyan"	
					#~ ;;
#~ esac
        lcOsSubSplClrStr="yellow/green cyan/blue"  # default to "00"  # Wierd Color Scheme
        lcOsSubSplClrID="Yellow/Green"
...
. 	                                    "&
    ...
    #=============================================================
    #! =============================================================
    #! function Menu.lst() {   ; n=0   ; } # end: Menu.lst
    #
	(
    echo "#!/bin/bash"
	echo "#=========================================="
...
. 					                "&
		...
		#
			...
			#=============== now
		#
fi # end: [ "$EXIT" = "ARB_ORG" ]    # EXIT:ARB_ORG
#!
#! ======================================================================
#! ## end: Long Menu   ################################################
#! ======================================================================
...
#(:
#! ###########################################################
#! function [EXIT:guiImageCntMax]
if [ "$EXIT" = 'guiImageCntMax' ] ; then n=0  ;
	n=0
	Sbr_KillProc	"${EXIT}------guiImageCntMax"
    Sbr_CDW_EGI_Long
    Sbr_Cbx_ReStart '' ### display
fi # end: "$EXIT" = 'guiImageCntMax'
#(!
#(: ====================================================================
#(: ft(EXIT:guiImageCntMin)
#(: called by: Main GUI Fall-Thru and code is INTERNAL to MGR
#(: param:
#(: purpose:
#(: calls:
...
#(:
#! ###########################################################
#! function [EXIT:guiImageCntMin]
if [ "$EXIT" = 'guiImageCntMin' ] ; then n=0  ;
	n=0
	Sbr_KillProc	"${EXIT}------guiImageCntMin"
    Sbr_CDW_EGI_Short
    Sbr_Cbx_ReStart '' ### display
fi # end: "$EXIT" = 'guiImageCntMin'
#(!
#!
#! #######################################################################
...
#!
#! #######################################################################
#! begin: #4 Column : System BootRebootQuit 
#! #######################################################################
#!
#(: ====================================================================
...
#(:
#! ###########################################################
#! function [EXIT:guiSetupQUIT]  # NOT USED
#~ if [ "$EXIT" = "guiSetupQUIT" ]  ; then
	#~ n=0
    #~ lcMsgStr="=== SetUp-QUIT ===" && Sbr_Str2DevTrm "${lcMsgStr}" "2" "7"
	#~ sleep 1
	#~ return
#~ fi
...
if [ "$EXIT" = "guiMainQUIT" ]  ; then
	...
	if [  -f   ${gcEmGaImgFnTrgTxtMaster} ] ; then 	# build EGI files in /tmp
        ...
        echo    # Str2DevTrm
        lcMsgStr="======== MGS_Main-Quit ========"    # text fg mrg
        ...
        echo 
#~ Sbr_Cbx_ReQuit   # "/usr/bin/wmpoweroff"
	exit 0  
    return  ### proper sequence must be "exit" then  "return" down the "stack"
#~ . /etc/rc.d/rc.z_WRMrestartwm.sh ### "return" down the "stack"
    #!
fi
#!
#!
